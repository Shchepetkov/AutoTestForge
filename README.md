...
- **Защита записи тестов.** Writer проверяет идентификаторы пакета и класса до создания каталогов и отклоняет символические ссылки и перенаправленные пути (включая junction в Windows) в пути назначения. Тест записывается во временный файл рядом с назначением и заменяет его атомарно, если файловая система поддерживает эту операцию; иначе используется обычное перемещение с заменой. Существующее поведение перезаписи обычных тестовых файлов сохраняется, но запись через жёсткую ссылку не меняет связанный файл. Эти проверки не являются изоляцией от другого процесса, одновременно подменяющего каталоги: для недоверенного проекта используйте отдельную рабочую копию с ограниченным доступом.
